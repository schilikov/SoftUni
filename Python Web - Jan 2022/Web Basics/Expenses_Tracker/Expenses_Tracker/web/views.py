...
def create_profile(request):
    if request.method == 'POST':
        form = CreateProfileForm(request.POST, request.FILES) # request.files for image files
        if form.is_valid():
            form.save()
            return redirect('show index')
    else:
...
def edit_profile(request):
    profile = get_profile()
    if request.method == 'POST':
        form = EditProfileForm(request.POST, request.FILES, instance=profile) # req.files for image files
        if form.is_valid():                                                     # instance for edit only
            form.save()
            return redirect('show index')
    else:
